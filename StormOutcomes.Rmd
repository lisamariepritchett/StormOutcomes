...
Now I can use this function with a list of Regular expressions to  match only the exact Event Type strings. All of the events without a label exactly matching one of the Event Types will be placed into a 'Not Categorized' group. This 
...
The keywords for each category are given in StormEventKeywords.csv
...
file = 'StormEventKeywords.csv'
GroupedEvents <- read.csv(file, stringsAsFactors = F)
...
### Labeling Specific Events Types
...
The result is a true Factor Variable with `r length(unique(My_StormData$Type))` levels

```{r makeCategoryFactor}
# Make factor and assign order
...
levels(My_StormData$Category) <- c( 'Tornado', 'Flood' , 'Ocean/Tidal', 
                                    'Thunderstorm', 'Winter Storm', 'Tropical Storm', 
                                    'Wind', 'Fire', 'Cold', 'Heat', 'Drought',
                                    'Debris Flow', 'Not Categorized')
```
